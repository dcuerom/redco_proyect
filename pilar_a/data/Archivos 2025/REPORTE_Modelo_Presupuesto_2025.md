...
| **Motor de proyección ("módulos")** | `Gantt Proyectos`, `EDP Proyectado`, `Ingreso Proyectado`, `Costos Proyectados por Unidad` (+ huérfanas `Ingreso Proyectado_Edu` y `Costo Directo Proyectos`) | Cálculo bottom-up por proyecto. Al `Flujo Caja` llegan **solo** `Ingreso Proyectado` (ingreso) y `Costos Proyectados por Unidad` (costo) ✱. |
...
| 8 | `Costo Directo Proyectos` | 211×42 | 6.383 | Costo por proyecto — **huérfana: nadie la lee** ✱ |
...
flowchart LR
    ...
    CD[Costo Directo Proyectos<br/>HUÉRFANO]:::orphan
...
flowchart TB
    ...
    G --> CU[Costos Proyectados por Unidad<br/>costo por país × mes<br/>detalle interno filas 63+]
    CD[Costo Directo Proyectos<br/>HUÉRFANO: nadie la lee]
    ING --> FC[Flujo Caja]
    CU --> FC[Flujo Caja]
```
...
- **`Costos Proyectados por Unidad`** es la hoja de costos que **sí** llega al flujo (§5). Construye el costo por **país** como un pivot de 2 niveles: la fila **Total** de cada país (Chile r13, Perú r21, Brasil r29, USA r37, Otros r45; gran total r53; columnas I:Q = abr–dic) es la suma de **7 categorías** (Personal, Oficina, Viajes, Capacitaciones, Ferias, Eventos, Otros), y cada categoría es un `SUMIFS` sobre una **tabla de detalle interna (filas 63+)** filtrada por país (col D) y categoría (col E). Ese detalle se alimenta de `Gantt Proyectos` + `Listas y Parametros`. **No lee `Costo Directo Proyectos`.**
- ✱ **`Costo Directo Proyectos` es una hoja huérfana.** Calcula esencialmente lo mismo (costo por país desde `Gantt`+`Listas`, con su propio pivot por país en columnas AF–AP y detalle en filas 29+), pero **ninguna otra hoja la referencia** (verificado por el grafo completo y por búsqueda de texto: solo se auto-referencia). Es el gemelo de costos del huérfano `Ingreso Proyectado_Edu`; **no entra al flujo de caja**.
...
| **Costos** | `='Costos Proyectados por Unidad'!I13` (Chile) · `!I21` (Perú) · `!I29` (Brasil) · `!I37` (USA) · `!I45` (Otros) | Costo por país × mes — **referencia directa a las filas-Total** de esa hoja (col I = abr … col Q = dic). |
...
| Ingresos (mes _m_) | `≈ EDP(m−2)` (lag 2 meses) | Budget |
...
3. ✱ **Dos hojas-motor huérfanas:** `Ingreso Proyectado_Edu` (clon de `Ingreso Proyectado`) y `Costo Directo Proyectos` (gemelo de costos de `Costos Proyectados por Unidad`). **Ninguna la consume**; el flujo usa `Ingreso Proyectado` y `Costos Proyectados por Unidad`. Riesgo de divergencia silenciosa entre cada par de gemelos.
