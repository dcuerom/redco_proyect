...
flowchart LR
    EX["Exploración\navanzada"] --> CO["Estudio\nConceptual\n(Scoping)"]
    CO --> PF["Prefactibilidad\n(PFS)"]
    PF --> FS["Factibilidad\n(FS / DFS)"]
    FS --> BE["Ingeniería\nBásica (FEED)"]
    BE --> DE["Ingeniería\nde Detalle"]
    DE --> EP["EPCM /\nEjecución"]
    EP --> CM["Comisionamiento\n& Ramp-up"]
    CM --> OP["Operación &\nOptimización"]
    OP -.->|debottlenecking,\nexpansión| CO

    subgraph Margen["Intensidad de margen del consultor"]
      direction TB
      M1["Alto margen / bajo volumen:\nConceptual · FS · Permitting · ESG"]
      M2["Volumen alto / margen medio:\nDetalle · EPCM"]
    end

    classDef hi fill:#009E73,stroke:#024,color:#fff;
    classDef mid fill:#0072B2,stroke:#024,color:#fff;
    class CO,PF,FS,BE hi;
    class DE,EP,CM mid;
```
...
```mermaid
xychart-beta
    title "Demanda de minerales seleccionados — índice (2024 = 100), escenario base"
    x-axis [2024, 2025, 2026, 2027, 2028, 2029, 2030]
    y-axis "Índice (2024 = 100)" 90 --> 270
    line [100, 117, 137, 160, 187, 219, 256]
    line [100, 109, 118, 129, 140, 152, 166]
    line [100, 106, 112, 119, 126, 133, 141]
    line [100, 103, 106, 109, 112, 116, 119]
    line [100, 101.5, 103, 104.6, 106.1, 107.7, 109.3]
```
...
```mermaid
xychart-beta
    title "Crecimiento absoluto de demanda 2024 a 2030 — Mt (eje log conceptual; acero a escala aparte)"
    x-axis ["Litio", "Cobalto", "Grafito", "Niquel", "Silicio", "Cobre"]
    y-axis "Demanda (Mt)" 0 --> 35
    bar [1.0, 0.20, 5.5, 3.3, 5.5, 27.0]
...
```mermaid
flowchart LR
    subgraph TECH["Energy Technologies (driver)"]
        EV["Vehículos\neléctricos (EV)"]
        BAT["Almacenamiento\npor baterías (BESS)"]
        PV["Solar\nfotovoltaica"]
        WIND["Eólica\n(on/offshore)"]
    ...
    subgraph MAT["Key Material Needs"]
        LI["Litio"]
        CO["Cobalto"]
        GR["Grafito"]
        NI["Níquel"]
    ...
    classDef t fill:#0072B2,stroke:#013,color:#fff;
    classDef m fill:#009E73,stroke:#013,color:#fff;
    classDef g fill:#E69F00,stroke:#013,color:#000;
    class EV,BAT,PV,WIND,GRID t;
    class LI,CO,GR,NI,ND,CU,AG,SI,AL,FE m;
    class G1,G2,G3,G4 g;
```
...
```mermaid
flowchart TD
    ROOT["Servicios de Ingeniería & Consultoría Minera\nFee pool 2030 base ≈ USD 22 mil M (100%)"]

    ROOT --> A["Estudios & Consultoría\n~25% · margen alto · CAGR ~10%"]
    ROOT --> B["Ingeniería\n~30% · margen medio-alto"]
    ROOT --> C["EPCM / Ejecución\n~35% · margen medio · procíclico"]
    ROOT --> D["Optimización Operacional\n~10% · margen alto · anticíclico"]

    A --> A1["Conceptual / Scoping"]
    A --> A2["PFS / FS / DFS"]
    A --> A3["Permitting · ESG · Cierre\n(mayor crecimiento de margen)"]

    B --> B1["Ingeniería Básica / FEED\n~12%"]
    B --> B2["Ingeniería de Detalle\n~18% · alta competencia precio"]

    C --> C1["Gestión EPCM"]
    C --> C2["Procurement & Construction Mgmt"]

    D --> D1["Debottlenecking / Expansión"]
    D --> D2["Eficiencia energética & descarbonización"]
    D --> D3["Digitalización / Asset performance"]

    classDef hi fill:#009E73,stroke:#013,color:#fff;
    classDef mid fill:#0072B2,stroke:#013,color:#fff;
    classDef low fill:#56B4E9,stroke:#013,color:#000;
    class A,A1,A2,A3,D,D1,D2,D3 hi;
    class B,B1 mid;
    class B2,C,C1,C2 low;
```
...
```mermaid
xychart-beta
    title "Frecuencia de riesgos por banda de severidad (S = P x I)"
    x-axis ["Bajo (1-6)", "Medio (7-12)", "Alto (13-18)", "Critico (19-25)"]
    y-axis "Nº de riesgos" 0 --> 8
    bar [0, 6, 3, 3]
```
...
```mermaid
xychart-beta
    title "Valor del mercado de servicios de ingenieria minera — 3 escenarios (USD mil M)"
    x-axis [2024, 2025, 2026, 2027, 2028, 2029, 2030]
    y-axis "USD mil millones" 12 --> 32
    line [14, 15.8, 17.9, 20.2, 22.9, 25.8, 29.2]
    line [14, 15.1, 16.3, 17.6, 19.0, 20.5, 22.2]
    line [14, 14.5, 15.0, 15.5, 16.0, 16.6, 17.2]
```
...
flowchart LR
    ...
    F1 --> F2 --> F3
    classDef p fill:#0072B2,stroke:#012,color:#fff;
    class F1,F2,F3 p;
```
